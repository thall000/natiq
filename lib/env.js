...
// Groq rate limits are per-model, not per-account — so the live conversation (latency-
// sensitive, called once per turn) and the two feedback routes (called once per
// answer/session, can tolerate a slower model) draw from the SAME quota bucket unless
// configured to use different models. These are independently overridable via env
// (default: the model this app has always used) so conversation traffic can't starve
// feedback generation, or vice versa, on a shared daily token limit.
export function getGroqConversationModel() {
  return process.env.GROQ_CONVERSATION_MODEL || "llama-3.3-70b-versatile";
}

export function getGroqFeedbackModel() {
  return process.env.GROQ_FEEDBACK_MODEL || "llama-3.3-70b-versatile";
}
